...
class ApkDiff:
    ...
    IGNORE_FILES = [
        ...
        "META-INF/SIGNAL_S.RSA",
        "META-INF/SIGNAL_S.SF",
        "resources.arsc",
    ]
    ...
    def compare(self, sourceApk, destinationApk):
        ...
        if (
            ...
            and self.compareEntries(sourceZip, destinationZip) == True
        ):
            print("APKs match!")
        else:
    ...
    def compareManifests(self, sourceZip, destinationZip):
        ...
        for (sourceEntryName, destinationEntryName) in zip(
            sourceEntrySortedList, destinationEntrySortedList
        ):
            if sourceEntryName != destinationEntryName:
                print(
                    "Sorted manifests don't match, %s vs %s"
                    % (sourceEntryName, destinationEntryName)
                )
                return False
    ...
    def compareEntries(self, sourceZip, destinationZip):
        sourceInfoList = list(
            filter(
                lambda sourceInfo: sourceInfo.filename not in self.IGNORE_FILES,
                sourceZip.infolist(),
            )
        )
        destinationInfoList = list(
            filter(
                lambda destinationInfo: destinationInfo.filename
                not in self.IGNORE_FILES,
                destinationZip.infolist(),
            )
        )
            ...
            for destinationEntryInfo in list(destinationInfoList):
                if sourceEntryInfo.filename == destinationEntryInfo.filename:
                    ...
                    if self.compareFiles(sourceEntry, destinationEntry) != True:
                        print(
                            "APK entry %s does not match %s!"
                            % (sourceEntryInfo.filename, destinationEntryInfo.filename)
                        )
                        return False
